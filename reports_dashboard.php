...
$monthlySubData = [];
for ($m = 1; $m <= 12; $m++) {
    $r = $conn->query("SELECT COALESCE(SUM(amount),0) as r FROM payments WHERE YEAR(payment_date) = YEAR(CURDATE()) AND MONTH(payment_date) = $m")->fetch_assoc()['r'];
    $rw = $conn->query("SELECT COALESCE(SUM(p.amount),0) as r FROM payments p LEFT JOIN members m ON p.member_id = m.member_id WHERE YEAR(p.payment_date) = YEAR(CURDATE()) AND MONTH(p.payment_date) = $m AND m.member_type = 'walkin'")->fetch_assoc()['r'];
    $rs = $conn->query("SELECT COALESCE(SUM(p.amount),0) as r FROM payments p LEFT JOIN members m ON p.member_id = m.member_id WHERE YEAR(p.payment_date) = YEAR(CURDATE()) AND MONTH(p.payment_date) = $m AND m.member_type = 'subscription'")->fetch_assoc()['r'];
    $monthlyRevData[] = (float)$r;
    $monthlyWalkinData[] = (float)$rw;
    $monthlySubData[] = (float)$rs;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['generate'])) {
    ...
    $overallRev = $conn->query("SELECT COALESCE(SUM(p.amount),0) as r FROM payments p LEFT JOIN members m ON p.member_id = m.member_id WHERE p.payment_date BETWEEN '$fromDate' AND '$toDate'$typeWhere")->fetch_assoc()['r'];
